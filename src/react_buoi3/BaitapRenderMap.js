...
export default class BaitapRenderMap extends Component {
    ...
    renderFilm = () => {
        const arrTagFilm = this.dataFilm.map((phim, index) => {
            return (
                <div key={index} className="col-4 mt-2">
                    <div className="card text-white bt-primary border-none">
                        <div style={{ backgroundImage: `url('${phim.hinhAnh}')`, backgroundPosition: 'center', backgroundSize: 'cover' }}>
                            <img style={{ opacity: 0 }} className="card-img-top" width="150" height="300" src={phim.hinhAnh} alt={phim.hinhAnh} />
                        </div>
                        <div className="card-body bg-dark">
                            <h4 className="card-title" style={{ height: 50 }}>
                                {phim.tenPhim.length > 50 ? <p>{phim.tenPhim.substring(0, 100)} ... </p> : <p>
                                    {phim.tenPhim} </p>}
                            </h4>
                            <div style={{ height: '80px' }} className="card-text">{phim.moTa.length > 100 ? <p>{phim.moTa.substr(0, 100)} ... </p> : <p>
                                {phim.moTa}</p>}
                            </div>
                        </div>
                    </div>
